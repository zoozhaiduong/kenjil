...
const AIChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: "Hello. I am Kenjil's virtual assistant. Ask me about my tools, experience, or how to contact me." }
  ]);
  ...
  const handleSend = async () => {
    ...
    // Format history for Gemini API (structure kept for compatibility)
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: "System offline. Please email kenjil.art@email.com" }]);
    } finally {
      setIsThinking(false);
    }
  };
      ...
      <div className={`fixed bottom-24 right-4 md:right-8 w-[90vw] md:w-96 bg-zinc-900/95 backdrop-blur-xl border border-zinc-700 rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 origin-bottom-right z-50 flex flex-col ${isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-90 translate-y-10 pointer-events-none'}`} style={{ height: '500px' }}>
        ...
        <div className="p-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-white text-sm">KENJIL AI</h3>
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-950/30">
          <div className="flex items-center gap-2">
            <input 
              ...
              placeholder="Ask about software, rates..."
              className="flex-1 bg-transparent border-none text-white text-sm focus:ring-0 placeholder-zinc-600 focus:outline-none"
            />
            <button 
              onClick={handleSend}
